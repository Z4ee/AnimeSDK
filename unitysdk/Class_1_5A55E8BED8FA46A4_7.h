#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/System/Object.h"

namespace Google::Protobuf { class CodedInputStream; }
namespace Google::Protobuf { class CodedOutputStream; }
namespace Google::Protobuf { class UnknownFieldSet; }
namespace Google::Protobuf { template <typename T> class FieldCodec_1; }
namespace Google::Protobuf { template <typename T> class MessageParser_1; }
namespace Google::Protobuf::Collections { template <typename T> class RepeatedField_1; }
namespace Google::Protobuf::Reflection { class MessageDescriptor; }
namespace System { class String; }

#define CLASS_1_5A55E8BED8FA46A4_7_CALCULATESIZE_OFFSET UNITYSDK_OFFSET(0x1E5E9C90)
#define CLASS_1_5A55E8BED8FA46A4_7_CLONE_OFFSET UNITYSDK_OFFSET(0x1E5E9770)
#define CLASS_1_5A55E8BED8FA46A4_7_EQUALS_1_OFFSET UNITYSDK_OFFSET(0x1E5E9810)
#define CLASS_1_5A55E8BED8FA46A4_7_EQUALS_OFFSET UNITYSDK_OFFSET(0x1E5E97E0)
#define CLASS_1_5A55E8BED8FA46A4_7_GETHASHCODE_OFFSET UNITYSDK_OFFSET(0x1E5E9930)
#define CLASS_1_5A55E8BED8FA46A4_7_MERGEFROM_1_OFFSET UNITYSDK_OFFSET(0x1E5E9E10)
#define CLASS_1_5A55E8BED8FA46A4_7_MERGEFROM_OFFSET UNITYSDK_OFFSET(0x1E5E9DA0)
#define CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_02A3D10A9D535169_OFFSET UNITYSDK_OFFSET(0x1E5E97A0)
#define CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_554199E0076172C5_1_OFFSET UNITYSDK_OFFSET(0x1E5E97D0)
#define CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_554199E0076172C5_OFFSET UNITYSDK_OFFSET(0x1E5E97C0)
#define CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_685747EAFC9F7288_OFFSET UNITYSDK_OFFSET(0x1E5E95B0)
#define CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_7786DC814C2FC3BE_OFFSET UNITYSDK_OFFSET(0x1E5E97B0)
#define CLASS_1_5A55E8BED8FA46A4_7_PB__GOOGLE_PROTOBUF_IMESSAGE_GET_DESCRIPTOR_OFFSET UNITYSDK_OFFSET(0x1E5E95E0)
#define CLASS_1_5A55E8BED8FA46A4_7_TOSTRING_OFFSET UNITYSDK_OFFSET(0x1E5E9B00)
#define CLASS_1_5A55E8BED8FA46A4_7_WRITETO_OFFSET UNITYSDK_OFFSET(0x1E5E9B60)
#define CLASS_1_5A55E8BED8FA46A4_7__CCTOR_OFFSET UNITYSDK_OFFSET(0x1E5E9F20)
#define CLASS_1_5A55E8BED8FA46A4_7__CTOR_1_OFFSET UNITYSDK_OFFSET(0x1E5E9680)
#define CLASS_1_5A55E8BED8FA46A4_7__CTOR_OFFSET UNITYSDK_OFFSET(0x1E5E95F0)

inline static constexpr unsigned int Class_1_5A55E8BED8FA46A4_7_TypeDefinitionIndex = 32062;

class Class_1_5A55E8BED8FA46A4_7 : public ::System::Object
{
public:
	static ::Google::Protobuf::MessageParser_1<::Class_1_5A55E8BED8FA46A4_7*>** StaticGet_CHLCMCKIGKG()
	{
		return (::Google::Protobuf::MessageParser_1<::Class_1_5A55E8BED8FA46A4_7*>**)Il2CppClass::FromTypeDefinitionIndex(Class_1_5A55E8BED8FA46A4_7_TypeDefinitionIndex)->GetStaticField(0x566C0);
	}
	static ::Google::Protobuf::FieldCodec_1<::System::UInt32>** StaticGet_DIFNGFODAED()
	{
		return (::Google::Protobuf::FieldCodec_1<::System::UInt32>**)Il2CppClass::FromTypeDefinitionIndex(Class_1_5A55E8BED8FA46A4_7_TypeDefinitionIndex)->GetStaticField(0x566C8);
	}
	static ::Google::Protobuf::FieldCodec_1<::System::UInt32>** StaticGet_NJOPDFMOJNJ()
	{
		return (::Google::Protobuf::FieldCodec_1<::System::UInt32>**)Il2CppClass::FromTypeDefinitionIndex(Class_1_5A55E8BED8FA46A4_7_TypeDefinitionIndex)->GetStaticField(0x566D0);
	}
	// static const ::System::Int32 BKCLJIHIJIJ = 0xA; // 0x0
	// static const ::System::Int32 NDFHHFGKMAI = 0x9; // 0x0
	// static const ::System::Int32 DFPPCAJDBFK = 0xE; // 0x0
	::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>* JFDCOLNGGJC; // 0x10
	::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>* EHMGJDCEJOE; // 0x18
	::Google::Protobuf::UnknownFieldSet* DHHIJOGAOFI; // 0x20
	::System::UInt32 HKMEMBIPHAN; // 0x28

	::System::Void _ctor()
	{
		return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7__CTOR_OFFSET))(this);
	}

	::System::Void _ctor_1(::Class_1_5A55E8BED8FA46A4_7* a1)
	{
		return ((::System::Void(*)(::PVOID, ::Class_1_5A55E8BED8FA46A4_7*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7__CTOR_1_OFFSET))(this, a1);
	}

	static ::System::Void _cctor()
	{
		return ((::System::Void(*)())((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7__CCTOR_OFFSET))();
	}

	static ::Google::Protobuf::MessageParser_1<::Class_1_5A55E8BED8FA46A4_7*>* Method_1_685747EAFC9F7288()
	{
		return ((::Google::Protobuf::MessageParser_1<::Class_1_5A55E8BED8FA46A4_7*>*(*)())((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_685747EAFC9F7288_OFFSET))();
	}

	::Google::Protobuf::Reflection::MessageDescriptor* pb__Google_Protobuf_IMessage_get_Descriptor()
	{
		return ((::Google::Protobuf::Reflection::MessageDescriptor*(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_PB__GOOGLE_PROTOBUF_IMESSAGE_GET_DESCRIPTOR_OFFSET))(this);
	}

	::Class_1_5A55E8BED8FA46A4_7* Clone()
	{
		return ((::Class_1_5A55E8BED8FA46A4_7*(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_CLONE_OFFSET))(this);
	}

	::System::UInt32 Method_1_02A3D10A9D535169()
	{
		return ((::System::UInt32(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_02A3D10A9D535169_OFFSET))(this);
	}

	::System::Void Method_1_7786DC814C2FC3BE(::System::UInt32 a1)
	{
		return ((::System::Void(*)(::PVOID, ::System::UInt32))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_7786DC814C2FC3BE_OFFSET))(this, a1);
	}

	::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>* Method_1_554199E0076172C5()
	{
		return ((::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>*(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_554199E0076172C5_OFFSET))(this);
	}

	::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>* Method_1_554199E0076172C5_1()
	{
		return ((::Google::Protobuf::Collections::RepeatedField_1<::System::UInt32>*(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_METHOD_1_554199E0076172C5_1_OFFSET))(this);
	}

	::System::Boolean Equals(::System::Object* a1)
	{
		return ((::System::Boolean(*)(::PVOID, ::System::Object*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_EQUALS_OFFSET))(this, a1);
	}

	::System::Boolean Equals_1(::Class_1_5A55E8BED8FA46A4_7* a1)
	{
		return ((::System::Boolean(*)(::PVOID, ::Class_1_5A55E8BED8FA46A4_7*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_EQUALS_1_OFFSET))(this, a1);
	}

	::System::Int32 GetHashCode()
	{
		return ((::System::Int32(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_GETHASHCODE_OFFSET))(this);
	}

	::System::String* ToString()
	{
		return ((::System::String*(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_TOSTRING_OFFSET))(this);
	}

	::System::Void WriteTo(::Google::Protobuf::CodedOutputStream* a1)
	{
		return ((::System::Void(*)(::PVOID, ::Google::Protobuf::CodedOutputStream*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_WRITETO_OFFSET))(this, a1);
	}

	::System::Int32 CalculateSize()
	{
		return ((::System::Int32(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_CALCULATESIZE_OFFSET))(this);
	}

	::System::Void MergeFrom(::Class_1_5A55E8BED8FA46A4_7* a1)
	{
		return ((::System::Void(*)(::PVOID, ::Class_1_5A55E8BED8FA46A4_7*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_MERGEFROM_OFFSET))(this, a1);
	}

	::System::Void MergeFrom_1(::Google::Protobuf::CodedInputStream* a1)
	{
		return ((::System::Void(*)(::PVOID, ::Google::Protobuf::CodedInputStream*))((::PBYTE)hIl2Cpp + CLASS_1_5A55E8BED8FA46A4_7_MERGEFROM_1_OFFSET))(this, a1);
	}
};
