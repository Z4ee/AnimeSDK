#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/Client/PhotoGraphAimUnionNodeType.h"
#include "unitysdk/RPG/GameCore/PhotoGraphAimIdentifyType.h"
#include "unitysdk/System/Object.h"
#include "unitysdk/UnityEngine/Vector3.h"

namespace RPG::GameCore { class GameEntity; }
namespace UnityEngine { class Animation; }
namespace UnityEngine { class Collider; }
namespace UnityEngine { class GameObject; }
namespace UnityEngine { class RectTransform; }
namespace UnityEngine { class Transform; }
namespace UnityEngine::UI { class Image; }
namespace UnityEngine::UI { class SmoothMask; }

#define RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM_SWITCHTONORMAL_OFFSET UNITYSDK_OFFSET(0xC3786F0)
#define RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM_SWITCHTOSUCCESS_OFFSET UNITYSDK_OFFSET(0xC3787D0)
#define RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM__CTOR_OFFSET UNITYSDK_OFFSET(0xC377640)

namespace RPG::Client
{
	inline static constexpr unsigned int PhotoGraphAimContainer_PhotoGraphAim_TypeDefinitionIndex = 64898;

	class PhotoGraphAimContainer_PhotoGraphAim : public ::System::Object
	{
	public:
		::UnityEngine::UI::SmoothMask* ProgressSM; // 0x10
		::UnityEngine::Animation* AnimationPending; // 0x18
		::UnityEngine::Transform* NodeSuccess; // 0x20
		::UnityEngine::UI::Image* NodeImage; // 0x28
		::UnityEngine::Transform* NodeFail; // 0x30
		::UnityEngine::Transform* TargetTransform; // 0x38
		::RPG::GameCore::GameEntity* TargetEntity; // 0x40
		::UnityEngine::Collider* TargetCollider; // 0x48
		::UnityEngine::Animation* AnimationBg; // 0x50
		::UnityEngine::RectTransform* Rect; // 0x58
		::UnityEngine::Transform* NodeStatus; // 0x60
		::UnityEngine::GameObject* ManualAimGO; // 0x68
		::UnityEngine::RectTransform* RectPending; // 0x70
		::UnityEngine::GameObject* PendingAimGO; // 0x78
		::System::Int32 UnionNodeIndex; // 0x80
		::RPG::GameCore::PhotoGraphAimIdentifyType IdentifyType; // 0x84
		::System::Boolean _IsSuccessStyle; // 0x88
		::System::Boolean UseAimUnion; // 0x89
		::System::Boolean ShowTag; // 0x8A
		::System::Boolean IsInUse; // 0x8B
		::System::Int32 UnionIndex; // 0x8C
		::System::Int32 HandlerIndex; // 0x90
		::RPG::Client::PhotoGraphAimUnionNodeType AimNodeType; // 0x94
		::UnityEngine::Vector3 ExtraAimTagOffset; // 0x98
		::System::Single ShowTagDirRange; // 0xA4
		::System::Int32 UnionNodeUnitIndex; // 0xA8
		::UnityEngine::Vector3 WorldPos; // 0xAC
		::UnityEngine::Vector3 TransformOffset; // 0xB8

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM__CTOR_OFFSET))(this);
		}

		::System::Void SwitchToNormal()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM_SWITCHTONORMAL_OFFSET))(this);
		}

		::System::Void SwitchToSuccess()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_CLIENT_PHOTOGRAPHAIMCONTAINER_PHOTOGRAPHAIM_SWITCHTOSUCCESS_OFFSET))(this);
		}
	};
}
