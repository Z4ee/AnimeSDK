#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/GameCore/TaskConfig.h"

class Class_1_7A22A3DBEEDD1F80;
namespace RPG::GameCore { class WeatherUIBaseConfig; }
namespace System { class String; }

#define RPG_GAMECORE_SETWEATHERUI_METHOD_3_938A4FFA2B5465F2_OFFSET UNITYSDK_OFFSET(0x1E095C80)
#define RPG_GAMECORE_SETWEATHERUI_METHOD_3_C4300B59F12ECFC1_OFFSET UNITYSDK_OFFSET(0x1E095C40)
#define RPG_GAMECORE_SETWEATHERUI__CTOR_OFFSET UNITYSDK_OFFSET(0x1E095C70)

namespace RPG::GameCore
{
	inline static constexpr unsigned int SetWeatherUI_TypeDefinitionIndex = 22532;

	class SetWeatherUI : public ::RPG::GameCore::TaskConfig
	{
	public:
		::RPG::GameCore::WeatherUIBaseConfig* Config; // 0x18
		::System::String* TypeName; // 0x20

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_GAMECORE_SETWEATHERUI__CTOR_OFFSET))(this);
		}

		static ::System::Void Method_3_C4300B59F12ECFC1(::Class_1_7A22A3DBEEDD1F80* a1, ::RPG::GameCore::SetWeatherUI*& a2)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::SetWeatherUI*&))((::PBYTE)hIl2Cpp + RPG_GAMECORE_SETWEATHERUI_METHOD_3_C4300B59F12ECFC1_OFFSET))(a1, a2);
		}

		static ::System::Void Method_3_938A4FFA2B5465F2(::Class_1_7A22A3DBEEDD1F80* a1, ::RPG::GameCore::SetWeatherUI* a2)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::SetWeatherUI*))((::PBYTE)hIl2Cpp + RPG_GAMECORE_SETWEATHERUI_METHOD_3_938A4FFA2B5465F2_OFFSET))(a1, a2);
		}
	};
}
