#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/GameCore/TaskConfig.h"

class Class_1_7A22A3DBEEDD1F80;

#define RPG_GAMECORE_CALLBACK_METHOD_3_8C55C0B1212F9650_OFFSET UNITYSDK_OFFSET(0x17051E90)
#define RPG_GAMECORE_CALLBACK_METHOD_3_95798EA89A492409_OFFSET UNITYSDK_OFFSET(0x17051F10)
#define RPG_GAMECORE_CALLBACK__CTOR_OFFSET UNITYSDK_OFFSET(0x17051EE0)

namespace RPG::GameCore
{
	inline static constexpr unsigned int CallBack_TypeDefinitionIndex = 19854;

	class CallBack : public ::RPG::GameCore::TaskConfig
	{
	public:
		::Il2CppArray<::RPG::GameCore::TaskConfig*>* OnDispose; // 0x18

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_GAMECORE_CALLBACK__CTOR_OFFSET))(this);
		}

		static ::System::Void Method_3_8C55C0B1212F9650(::Class_1_7A22A3DBEEDD1F80* a1, ::RPG::GameCore::CallBack*& a2)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::CallBack*&))((::PBYTE)hIl2Cpp + RPG_GAMECORE_CALLBACK_METHOD_3_8C55C0B1212F9650_OFFSET))(a1, a2);
		}

		static ::System::Void Method_3_95798EA89A492409(::Class_1_7A22A3DBEEDD1F80* a1, ::RPG::GameCore::CallBack* a2)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::CallBack*))((::PBYTE)hIl2Cpp + RPG_GAMECORE_CALLBACK_METHOD_3_95798EA89A492409_OFFSET))(a1, a2);
		}
	};
}
