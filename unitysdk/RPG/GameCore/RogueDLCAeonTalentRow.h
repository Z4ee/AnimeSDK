#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/Client/TextID.h"
#include "unitysdk/RPG/GameCore/FixPoint.h"
#include "unitysdk/System/Object.h"

class Class_1_7A22A3DBEEDD1F80;
namespace System { class String; }

#define RPG_GAMECORE_ROGUEDLCAEONTALENTROW_FROMBINARY_OFFSET UNITYSDK_OFFSET(0x19B28400)
#define RPG_GAMECORE_ROGUEDLCAEONTALENTROW__CTOR_OFFSET UNITYSDK_OFFSET(0x19B28C30)

namespace RPG::GameCore
{
	inline static constexpr unsigned int RogueDLCAeonTalentRow_TypeDefinitionIndex = 14016;

	class RogueDLCAeonTalentRow : public ::System::Object
	{
	public:
		::Il2CppArray<::RPG::GameCore::FixPoint>* EffectDescParamList; // 0x10
		::System::String* TalentIcon; // 0x18
		::Il2CppArray<::System::UInt32>* GamePlayEffectList; // 0x20
		::System::Boolean IsImportant; // 0x28
		::System::UInt32 UnlockAeonDimensionPoint; // 0x2C
		::RPG::Client::TextID EffectTitle; // 0x30
		::RPG::Client::TextID EffectDesc; // 0x40
		::System::UInt32 AeonTalentID; // 0x50
		::System::UInt32 AeonDimensionID; // 0x54

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + RPG_GAMECORE_ROGUEDLCAEONTALENTROW__CTOR_OFFSET))(this);
		}

		static ::System::Void FromBinary(::Class_1_7A22A3DBEEDD1F80* a1, ::RPG::GameCore::RogueDLCAeonTalentRow*& a2)
		{
			return ((::System::Void(*)(::Class_1_7A22A3DBEEDD1F80*, ::RPG::GameCore::RogueDLCAeonTalentRow*&))((::PBYTE)hIl2Cpp + RPG_GAMECORE_ROGUEDLCAEONTALENTROW_FROMBINARY_OFFSET))(a1, a2);
		}
	};
}
