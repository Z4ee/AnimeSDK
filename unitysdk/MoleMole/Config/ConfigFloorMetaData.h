#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/FlexBuffers/FlxMap.h"
#include "unitysdk/FlexBuffers/FlxValue.h"
#include "unitysdk/System/Object.h"

class Class_1_236FD73ABC492E3F;
namespace System { class String; }

#define MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_FROMBINARY_OFFSET UNITYSDK_OFFSET(0x1625DD10)
#define MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_FROMFLX_OFFSET UNITYSDK_OFFSET(0x1625D820)
#define MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_METHOD_1_8741A88D50268453_OFFSET UNITYSDK_OFFSET(0x1625D8A0)
#define MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_METHOD_1_BF50AF6BCBDE0EBC_OFFSET UNITYSDK_OFFSET(0x1625DD70)
#define MOLEMOLE_CONFIG_CONFIGFLOORMETADATA__CTOR_OFFSET UNITYSDK_OFFSET(0x1625E0B0)

namespace MoleMole::Config
{
	inline static constexpr unsigned int ConfigFloorMetaData_TypeDefinitionIndex = 44048;

	class ConfigFloorMetaData : public ::System::Object
	{
	public:
		::System::Int32 MonsterPlayType; // 0x10
		::System::String* GlobalAIKey; // 0x18
		::System::Boolean DisableEntityDelayCreation; // 0x20

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_CONFIG_CONFIGFLOORMETADATA__CTOR_OFFSET))(this);
		}

		::System::Boolean FromFlx(::FlexBuffers::FlxValue a1)
		{
			return ((::System::Boolean(*)(::PVOID, ::FlexBuffers::FlxValue))((::PBYTE)hIl2Cpp + MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_FROMFLX_OFFSET))(this, a1);
		}

		::System::Boolean FromBinary(::Class_1_236FD73ABC492E3F* a1, ::System::Int32 a2)
		{
			return ((::System::Boolean(*)(::PVOID, ::Class_1_236FD73ABC492E3F*, ::System::Int32))((::PBYTE)hIl2Cpp + MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_FROMBINARY_OFFSET))(this, a1, a2);
		}

		::System::Boolean Method_1_BF50AF6BCBDE0EBC(::Class_1_236FD73ABC492E3F* a1, ::System::Int32 a2)
		{
			return ((::System::Boolean(*)(::PVOID, ::Class_1_236FD73ABC492E3F*, ::System::Int32))((::PBYTE)hIl2Cpp + MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_METHOD_1_BF50AF6BCBDE0EBC_OFFSET))(this, a1, a2);
		}

		::System::Boolean Method_1_8741A88D50268453(::FlexBuffers::FlxMap a1)
		{
			return ((::System::Boolean(*)(::PVOID, ::FlexBuffers::FlxMap))((::PBYTE)hIl2Cpp + MOLEMOLE_CONFIG_CONFIGFLOORMETADATA_METHOD_1_8741A88D50268453_OFFSET))(this, a1);
		}
	};
}
