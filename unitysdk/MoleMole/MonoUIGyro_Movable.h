#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/System/Object.h"

namespace UnityEngine { class Transform; }

#define MOLEMOLE_MONOUIGYRO_MOVABLE__CTOR_OFFSET UNITYSDK_OFFSET(0x13AC5DE0)

namespace MoleMole
{
	inline static constexpr unsigned int MonoUIGyro_Movable_TypeDefinitionIndex = 59963;

	class MonoUIGyro_Movable : public ::System::Object
	{
	public:
		::UnityEngine::Transform* transform; // 0x10
		::System::Single rangeRatio; // 0x18
		::System::Single speedRatio; // 0x1C

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_MONOUIGYRO_MOVABLE__CTOR_OFFSET))(this);
		}
	};
}
