#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/System/Object.h"

namespace MoleMole { class TypeTextComponent; }
namespace System { class String; }

#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_MOVENEXT_OFFSET UNITYSDK_OFFSET(0x1586A350)
#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_GENERIC_IENUMERATOR_SYSTEM_OBJECT__GET_CURRENT_OFFSET UNITYSDK_OFFSET(0x1586BF40)
#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_IENUMERATOR_GET_CURRENT_OFFSET UNITYSDK_OFFSET(0x1586BFA0)
#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_IENUMERATOR_RESET_OFFSET UNITYSDK_OFFSET(0x1586BF50)
#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_IDISPOSABLE_DISPOSE_OFFSET UNITYSDK_OFFSET(0x1586A340)
#define MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8__CTOR_OFFSET UNITYSDK_OFFSET(0x1586A330)

namespace MoleMole
{
	inline static constexpr unsigned int TypeTextComponent_Class_1_1CE966DD71EFF4F8_TypeDefinitionIndex = 81766;

	class TypeTextComponent_Class_1_1CE966DD71EFF4F8 : public ::System::Object
	{
	public:
		::System::String* Field_1_3; // 0x10
		::MoleMole::TypeTextComponent* Field_1_2; // 0x18
		::System::String* Field_1_7; // 0x20
		::System::Object* Field_1_1; // 0x28
		::System::Int32 Field_1_0; // 0x30
		::System::Int32 Field_1_4; // 0x34
		::System::Boolean Field_1_6; // 0x38
		::System::Single Field_1_5; // 0x3C
		::System::Int32 Field_1_8; // 0x40

		::System::Void _ctor(::System::Int32 a1)
		{
			return ((::System::Void(*)(::PVOID, ::System::Int32))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8__CTOR_OFFSET))(this, a1);
		}

		::System::Void System_IDisposable_Dispose()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_IDISPOSABLE_DISPOSE_OFFSET))(this);
		}

		::System::Boolean MoveNext()
		{
			return ((::System::Boolean(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_MOVENEXT_OFFSET))(this);
		}

		::System::Object* System_Collections_Generic_IEnumerator_System_Object__get_Current()
		{
			return ((::System::Object*(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_GENERIC_IENUMERATOR_SYSTEM_OBJECT__GET_CURRENT_OFFSET))(this);
		}

		::System::Void System_Collections_IEnumerator_Reset()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_IENUMERATOR_RESET_OFFSET))(this);
		}

		::System::Object* System_Collections_IEnumerator_get_Current()
		{
			return ((::System::Object*(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_TYPETEXTCOMPONENT_CLASS_1_1CE966DD71EFF4F8_SYSTEM_COLLECTIONS_IENUMERATOR_GET_CURRENT_OFFSET))(this);
		}
	};
}
