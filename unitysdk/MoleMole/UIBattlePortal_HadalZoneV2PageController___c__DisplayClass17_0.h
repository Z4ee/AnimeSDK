#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/System/Object.h"

class Class_1_E9529ED64113E4E9;
namespace UnityEngine::UI { class RawImage; }

#define MOLEMOLE_UIBATTLEPORTAL_HADALZONEV2PAGECONTROLLER___C__DISPLAYCLASS17_0__CTOR_OFFSET UNITYSDK_OFFSET(0x1931A3C0)
#define MOLEMOLE_UIBATTLEPORTAL_HADALZONEV2PAGECONTROLLER___C__DISPLAYCLASS17_0___PLAYBACKGROUNDVIDEO_B__0_OFFSET UNITYSDK_OFFSET(0x1931A3D0)

namespace MoleMole
{
	inline static constexpr unsigned int UIBattlePortal_HadalZoneV2PageController___c__DisplayClass17_0_TypeDefinitionIndex = 70037;

	class UIBattlePortal_HadalZoneV2PageController___c__DisplayClass17_0 : public ::System::Object
	{
	public:
		::UnityEngine::UI::RawImage* _videoRawImg; // 0x10

		::System::Void _ctor()
		{
			return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + MOLEMOLE_UIBATTLEPORTAL_HADALZONEV2PAGECONTROLLER___C__DISPLAYCLASS17_0__CTOR_OFFSET))(this);
		}

		::System::Void __PlayBackgroundVideo_b__0(::Class_1_E9529ED64113E4E9* vp)
		{
			return ((::System::Void(*)(::PVOID, ::Class_1_E9529ED64113E4E9*))((::PBYTE)hIl2Cpp + MOLEMOLE_UIBATTLEPORTAL_HADALZONEV2PAGECONTROLLER___C__DISPLAYCLASS17_0___PLAYBACKGROUNDVIDEO_B__0_OFFSET))(this, vp);
		}
	};
}
