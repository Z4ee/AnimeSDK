#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/System/ValueType.h"
#include "unitysdk/UnityEngine/Vector2Int.h"

namespace System { class String; }
namespace UnityEngine { class Texture2D; }

inline static constexpr unsigned int Struct_2_1DFA7EEEC04804D1_TypeDefinitionIndex = 94844;

struct alignas(8) Struct_2_1DFA7EEEC04804D1
{
	::UnityEngine::Texture2D* Field_2_0; // 0x10
	::UnityEngine::Vector2Int Field_2_7; // 0x18
	::System::Int32 Field_2_6; // 0x20
	::System::Int32 Field_2_5; // 0x24
	::System::Boolean Field_2_4; // 0x28
	::System::Int32 Field_2_11; // 0x2C
	::System::String* Field_2_10; // 0x30
};
