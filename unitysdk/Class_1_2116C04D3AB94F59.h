#pragma once
#include "unitysdk/unitysdk.h"
#include "unitysdk/RPG/GameCore/RtBehaviorFlag.h"
#include "unitysdk/System/Object.h"

class Class_3_07C3C4D2990C49EE;

#define CLASS_1_2116C04D3AB94F59__CTOR_OFFSET UNITYSDK_OFFSET(0x16049F30)

inline static constexpr unsigned int Class_1_2116C04D3AB94F59_TypeDefinitionIndex = 54320;

class Class_1_2116C04D3AB94F59 : public ::System::Object
{
public:
	::Class_3_07C3C4D2990C49EE* GGNBOEIMEPM; // 0x10
	::Class_3_07C3C4D2990C49EE* GAFKECIDLJG; // 0x18
	::RPG::GameCore::RtBehaviorFlag FNPGDNFLMPC; // 0x20

	::System::Void _ctor()
	{
		return ((::System::Void(*)(::PVOID))((::PBYTE)hIl2Cpp + CLASS_1_2116C04D3AB94F59__CTOR_OFFSET))(this);
	}
};
